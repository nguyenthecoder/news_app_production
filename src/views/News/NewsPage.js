...
import { WATCHLIST_FETCHED, WATCHLIST_UPDATED } from '../../redux/actions'
...
const NewsPage = (props) => {
  ...
  const findSticker = (description) => {
    ...
    for (const watchlist of watchlistState.watchlist) {
      ...
      watchlistDescription = watchlistDescription.toLowerCase()
      watchlistDescription = watchlistDescription.replace('com', '')
      watchlistDescription = watchlistDescription.replace('inc', '')
      watchlistDescription = watchlistDescription.trim(0)
      if (watchlistDescription === description.toLowerCase()) {
        return watchlist
      }
    }
    return null
      ...
      <ContentBody open={true} marginTop={65} style={{ flex: 1 }}>
        {
          headlinePage && (
            <React.Fragment>
              ...
              {
                !loading && Object.keys(news).filter(description => findSticker(description) !== null).map((description, index) => {
